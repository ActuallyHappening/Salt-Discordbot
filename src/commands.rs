...
pub fn commands() -> Vec<twilight_model::application::command::Command> {
  [
    send_email::SendEmail::create_command().into(),
    orders::OrdersListCommand::create_command().into(),
  ]
  .into_iter()
  .collect()
}
...
pub async fn handle_command(
  ...
  match &*data.name {
    "send-email" => send_email::SendEmail::handle(interaction, data, state.get().client).await,
...
mod orders {
  ...
  impl OrdersListCommand {
    pub async fn handle(
      ...
      let command = Self::from_interaction(data.into()).wrap_err("failed to parse command data")?;
